...
- Phase 2.1 and 2.2 are committed. Phase 2.3 policy reads, atomic transitions,
  entitlement checks, and safe new-Store defaults are verified; its commit gate
  is pending. Later subphases have not started.
...
| 2 | Policy, authorization, and Customer association foundation | 2.1–2.5 | Commit gate (2.3) | Every policy mutation, Customer association, and send path is backend-authorized. |
...
complete. Phase 2.2 implementation and review are complete and committed.
Phase 2.3 implementation, verification, and standards/spec review are
complete; its commit gate is pending. Later Phase 2 subphases remain blocked
until that gate. Phases 8 and 9 are intentionally deferred from the initial
release.
