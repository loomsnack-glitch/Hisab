...
**What to build:** Give Ganatri owners a separate, secure path into the Platform Operations Console. An operator can seed the first active Owner User with a password-safe CLI command; Owner Users can then sign in with password or WhatsApp OTP to a new platform-admin application. The resulting owner session is isolated from customer User and Store Device sessions, and an inactive Owner User loses access on their next request.
...
- [ ] The new platform-admin application presents owner login and an authenticated console entry state with appropriate loading, invalid-credential, and expired-session handling.
