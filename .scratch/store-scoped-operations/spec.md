...
A Store Product Offering controls whether an existing Catalog Product appears in one Store's menu and owns that Store's selling price, discount, and active/inactive status. A Store Vendor Availability is the local purchasing switch for an existing Vendor at one Store: every Vendor × Store pair exists, and status is active or inactive. A Store Vendor Item Offering owns the default purchase price of an existing Vendor Item at one Store and retains that price while the Vendor is inactive there.
...
11. As an Organization administrator, I want to create a Vendor once and have it appear at every current Store as an active Store Vendor Availability, so that I do not have to pick target Stores or add it later.
12. As an Organization administrator, I want a newly created Store to inherit every existing Vendor as an inactive Store Vendor Availability, so that a new branch cannot buy from suppliers until someone activates them.
13. As an Organization administrator, I want to deactivate a Store Vendor Availability rather than unassign it, so that the Store cannot start new Purchases from that Vendor without losing the Vendor or its Store-specific default prices.
...
- Extend the existing Catalog module with Store Product Offerings. The module owns creation, listing, updating, activation, deactivation, and validation of product availability for a Store.
...
- Extend the existing Vendors module with Store Vendor Availability and Store Vendor Item Offerings. The module owns Vendor × Store matrix expansion, Store Vendor Item default-price maintenance, Store-filtered listings, and Purchase eligibility validation.
- A Store Vendor Availability references one Organization Vendor and one Store in the same Organization. Every Vendor has exactly one Availability per Store. Stores never create private Vendors or vendor approval requests. There is no target-Store picker and no API to create or delete an Availability after the fact.
- A Store Vendor Item Offering references an existing Vendor Item and an eligible Store Vendor Availability. It owns that Store's default purchase price and retains it while the Vendor is inactive at that Store. The agreed unit price on a Purchase Line remains authoritative historical data and may differ from the default.
- Creating a Vendor writes one active Store Vendor Availability for every current Store in the Organization. Creating a Store writes one inactive Availability for every existing Vendor. Vendor Item creation/configuration remains Organization-owned; creating a Vendor Item writes a Store Vendor Item Offering for every Store where its Vendor has an Availability.
- Adapt Purchase creation and editing to list and accept only Vendors whose Store Vendor Availability is active, and only their eligible Vendor Items. The backend rejects a Vendor or Vendor Item that is inactive at the Store, missing an Offering, from another Organization, or used with the wrong Store.
...
- Store Catalog scope is owned as follows, and must not be implied merely by adding Store pages:
  - Products: Store price, discount, and active/inactive status through Store Product Offerings.
  - Categories: shared Organization structure; show read-only in Store context unless Store category visibility is explicitly required.
  - Add-Ons: a Store Add-On Offering is required before a Store can have local price, discount, or status; that is a real backend/data-model feature, not a UI filter, and remains out of scope until that model exists.
  - Labels: shared templates and product label facts; if labels display selling price, resolve the selected Store's effective Product Offering at preview/print time.
...
- Add an explicit Organization workspace and selected Store workspace to Ganatri Admin navigation. The Organization workspace owns Stores, shared Catalog Product creation and details, shared Vendor creation and details, Units, Customers, and all-Store reports. The selected Store workspace has no landing-page button grid; its root URL redirects to Store Products, and module navigation is sidebar-first. Store Products and Store Vendors are the first Store sidebar destinations. Later Store modules—Billing, Purchases, Tables, Expenses, Money, Reports—follow the same sidebar-first model.
...
- Add Vendors service, route, and Purchase integration tests for complete Vendor × Store coverage, Store-specific Vendor Item default price selection, differing Store defaults for the same Vendor Item, actual Purchase Line override and snapshot behavior, retained prices after deactivate/reactivate, and rejection of inactive Store Vendor Availability or missing Offering use.
...
- Add Admin route/page behavior tests for context switching, Organization versus Store navigation, Store workspace root redirect to Store Products, Store sidebar Products and Vendors, Product creation without target-Store selection, Store Offering editing, Store Vendor activate/deactivate without assign/unassign, no Catalog Product/Vendor creation control in the Store workspace, and no Organization-scoped sidebar links while a Store workspace is selected.
...
- Store-specific Add-On price, discount, or status rules until a Store Add-On Offering data model exists. Inventory, stock transfers, vendor approval workflows, supplier contracts, procurement orders, or advanced purchasing permissions.
