...
import { getWhatsAppCloudTemplateSubmissions, getWhatsAppCloudTemplates, importWhatsAppCloudTemplateForStore, setWhatsAppCloudTemplateDefault, submitWhatsAppCloudTemplate, syncWhatsAppCloudTemplates } from "@repo/services";
import { WHATSAPP_DEFAULT_TEMPLATE_BODIES, type WhatsAppCloudAccountSnapshot, type WhatsAppMessageTemplateKind } from "@repo/types";
...
const defaultTokenNames: Record<WhatsAppMessageTemplateKind, string[]> = {
    bill: ["customer_name", "bill_number", "total", "paid", "balance_due", "organization_name", "store_name"],
...
const defaultBody = (kind: WhatsAppMessageTemplateKind): string => {
    const source = WHATSAPP_DEFAULT_TEMPLATE_BODIES[kind] ?? "Hello {{customer_name}}";
...
const WhatsAppCloudTemplateManager = ({ organizationId, storeId, accounts }: Props) => {
    ...
    const [importKinds, setImportKinds] = useState<Record<string, WhatsAppMessageTemplateKind>>({});
    ...
    const cards = useMemo(() => {
        ...
        const assets = cloudTemplates.map(template => ({ id: `asset-${template.id}`, submissionId: byProviderId.get(template.metaTemplateId)?.id ?? null, submissionStatus: byProviderId.get(template.metaTemplateId)?.status ?? null, cloudTemplateId: template.id, name: template.name, language: template.languageCode, kind: byProviderId.get(template.metaTemplateId)?.kind ?? null, category: template.category, status: template.status, reason: template.rejectionReason }));
        ...
        return [...assets, ...pending.map(item => ({ id: item.id, submissionId: item.id, submissionStatus: item.status, cloudTemplateId: null, name: item.friendlyName, language: item.languageCode, kind: item.kind, category: item.category, status: item.status, reason: item.rejectionReason ?? item.lastErrorMessage }))];
    ...
    const defaultMutation = useMutation({
        mutationFn: (submissionId: string) => setWhatsAppCloudTemplateDefault(organizationId, submissionId),
        onSuccess: response => { if (response.status !== "success") toast.error(response.message); else { invalidate(); toast.success(response.message); } },
        onError: () => toast.error("Template could not become the Store default"),
    });
    const importMutation = useMutation({
        mutationFn: (input: { cloudTemplateId: string; whatsappBusinessAccountId: string; kind: WhatsAppMessageTemplateKind }) => importWhatsAppCloudTemplateForStore(organizationId, storeId, input),
        onSuccess: response => { if (response.status !== "success") toast.error(response.message); else { invalidate(); toast.success(response.message); } },
        onError: () => toast.error("Existing Cloud template could not be assigned"),
    });
    const openCreate = () => { setKind("bill"); setFriendlyName(""); setLanguageCode("en_US"); setBody(defaultBody("bill")); setFooter(""); setUrlButton(""); setSampleValues(defaultSampleValues.bill); setOpen(true); };
    const duplicateSubmission = (submissionId: string) => {
        const submission = submissions.find(item => item.id === submissionId);
        if (!submission) return;
        const bodyComponent = submission.requestedComponents.find(component => component && typeof component === "object" && !Array.isArray(component) && String((component as Record<string, unknown>).type).toUpperCase() === "BODY") as Record<string, unknown> | undefined;
        const footerComponent = submission.requestedComponents.find(component => component && typeof component === "object" && !Array.isArray(component) && String((component as Record<string, unknown>).type).toUpperCase() === "FOOTER") as Record<string, unknown> | undefined;
        setKind(submission.kind);
        setFriendlyName(`${submission.friendlyName} copy`);
        setLanguageCode(submission.languageCode);
        setBody(typeof bodyComponent?.text === "string" ? bodyComponent.text : defaultBody(submission.kind));
        setFooter(typeof footerComponent?.text === "string" ? footerComponent.text : "");
        setUrlButton("");
        setSampleValues(Object.keys(submission.sampleValues).sort((a, b) => Number(a) - Number(b)).map(key => String(submission.sampleValues[key] ?? "")).join("|"));
        setOpen(true);
    };
    ...
    return <div className="space-y-4 rounded-xl border border-border/60 bg-background/50 p-3 sm:p-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">{cards.map(card => <div key={card.id} className="space-y-2 rounded-xl border border-border/60 bg-background/70 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{card.name}</p><p className="text-xs text-muted-foreground">{card.kind ? kinds.find(item => item.value === card.kind)?.label : "Existing Cloud template"} · {card.language}</p></div>{statusBadge(card.status)}</div>{card.reason ? <p className="flex gap-1.5 text-xs text-destructive"><ShieldAlert className="mt-0.5 size-3.5 shrink-0" />{card.reason}</p> : null}{card.status === "approved" && card.submissionId && card.submissionStatus === "approved" ? <div className="flex flex-wrap items-center justify-between gap-2"><p className="flex items-center gap-1.5 text-xs text-emerald-700"><CheckCircle2 className="size-3.5" />Approved</p><Button type="button" size="sm" variant="outline" className="h-8 rounded-lg text-xs" disabled={defaultMutation.isPending} onClick={() => defaultMutation.mutate(card.submissionId!)}>Set as Store default</Button></div> : card.status === "approved" && card.cloudTemplateId ? <div className="flex flex-wrap items-center gap-2"><Select value={importKinds[card.id] ?? (card.kind ?? (card.category === "marketing" ? "promotion" : "bill"))} onValueChange={value => setImportKinds(current => ({ ...current, [card.id]: value as WhatsAppMessageTemplateKind }))}><SelectTrigger className="h-8 min-w-32 rounded-lg text-xs" aria-label={`Message type for ${card.name}`}><SelectValue>{kinds.find(item => item.value === (importKinds[card.id] ?? (card.kind ?? (card.category === "marketing" ? "promotion" : "bill"))))?.label ?? "Choose message type"}</SelectValue></SelectTrigger><SelectContent>{kinds.filter(item => item.category.toLowerCase() === (card.category === "marketing" ? "marketing" : "utility").toLowerCase()).map(item => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select><Button type="button" size="sm" variant="outline" className="h-8 rounded-lg text-xs" disabled={importMutation.isPending} onClick={() => importMutation.mutate({ cloudTemplateId: card.cloudTemplateId!, whatsappBusinessAccountId: businessAccountId, kind: importKinds[card.id] ?? (card.kind ?? (card.category === "marketing" ? "promotion" : "bill")) })}>Use for Store</Button></div> : card.status === "rejected" || card.status === "failed" ? <Button type="button" size="sm" variant="outline" className="h-8 rounded-lg text-xs" onClick={() => duplicateSubmission(card.submissionId ?? card.id)}>Duplicate and edit</Button> : card.status === "pending" || card.status === "submitting" ? <p className="text-xs text-muted-foreground">Meta approval is required before sending.</p> : null}</div>)}</div>
