...
import type { UnitDTO, UnitKind } from "@repo/types";
...
import { LayoutGrid, Pencil, PlusCircle, RefreshCw, Ruler, Search, Table as TableIcon, X } from "lucide-react";
...
const UnitsPage = () => {
    ...
    const filteredUnits = useMemo(() => {
        if (!mobileSearchQuery.trim()) return units;
    ...
    const columns = useMemo<ColumnDef<UnitDTO>[]>(() => [
        {
            ...
            sortable: true,
            getSortValue: (unit) => unit.name,
        },
        {
            id: "kind",
            header: "Source",
            accessor: (unit) => <UnitKindBadge kind={unit.kind} />,
            sortable: true,
            getSortValue: (unit) => unitKindLabel[unit.kind],
            filterOptions: [
                { label: "Standard", value: "predefined" },
                { label: "Custom", value: "custom" },
            ],
            getFilterValue: (unit) => unit.kind,
        },
        {
            id: "status",
            header: "Availability",
            accessor: (unit) => <ProductStatusBadge status={unit.status} />,
            sortable: true,
            getSortValue: (unit) => unit.status,
            filterOptions: [
                { label: "Active", value: "active" },
                { label: "Inactive", value: "inactive" },
            ],
            getFilterValue: (unit) => unit.status,
        },
        {
            id: "updatedAt",
            header: "Updated",
            accessor: (unit) => formatDateTime(unit.updatedAt),
            sortable: true,
            getSortValue: (unit) => String(unit.updatedAt),
        },
    ], []);

    const renderActions = (unit: UnitDTO) => (
        <UpsertUnitDialog
                ...
                <>
                    <div className="hidden sm:block">
                        <PremiumTable
                            ...
                            infoText={`${units.length} unit${units.length === 1 ? "" : "s"}`}
                            toolbarActions={
                                <UpsertUnitDialog
                                    organizationId={organizationId}
                                    trigger={
                                        ...
                                        >
                                            ...
                                            <div className="mt-3 flex items-center justify-between border-t border-border/40 pt-2.5">
                                                <p className="text-[11px] text-muted-foreground">
                                                    Updated {formatDateTime(unit.updatedAt)}
                                                </p>
                                                <UpsertUnitDialog
                                                    organizationId={organizationId}
                                                    unit={unit}
