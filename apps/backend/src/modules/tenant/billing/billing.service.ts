    ...
    if (!isWholeCount(quantity)) {
      return {
        error: {
          status: "error",
          message:
            "Combo option quantity must be a whole number greater than 0",
          data: null,
          code: STATUS_CODES.BAD_REQUEST,
        },
      };
    ...
    if (seen.has(key)) {
      return {
        error: {
          status: "error",
          message:
            "The same Combo option cannot be selected twice in one configuration",
          data: null,
          code: STATUS_CODES.BAD_REQUEST,
        },
      };
...
const mergeSaleItemInputsByConfiguration = (
  ...
  for (const item of items) {
    ...
    const normalizedComboSelections = normalizeComboSelections(
...
export const getSaleNumberSettings = async (
  ...
  const existing = await billingRepository.getSaleNumberSettings(
    organizationId,
    storeId,
  );
  const settings =
...
const rescaleFrozenConfiguredLine = (
  ...
  const lineSubtotal = roundMoney(
    parentQuantity * (unitPrice + configuredExtraPerUnit),
  );
...
> => {
  ...
  const optionsByKey = new Map(
    options.map((option) => [
      `${option.choiceGroupId}:${option.optionProductId}`,
      option,
    ]),
  );
  ...
  for (const group of groups) {
    ...
    const totalSelections = groupSelections.reduce(
      (total, selection) => total + selection.quantity,
      0,
    );
    if (
      totalSelections < group.minSelections ||
      totalSelections > group.maxSelections
    ) {
      return {
        error: {
          status: "error",
    ...
    for (const selection of groupSelections) {
      const option = optionsByKey.get(
        `${group.id}:${selection.optionProductId}`,
      );
      ...
      const optionProduct = await catalogRepository.getProductById(
        organizationId,
        option.optionProductId,
      );
      if (
        !optionProduct ||
        optionProduct.status !== "active" ||
        optionProduct.productType !== "single"
      ) {
        return {
          error: {
            status: "error",
  ...
  const unitPrice = moneyFrom(product.price);
  const lineSubtotal = roundMoney(
    parentQuantity * unitPrice + adjustmentSubtotal + optionAddOnSubtotal,
  );
  const discountAmount = roundMoney(
  ...
  for (const line of lines) {
    ...
    for (const bundleComponent of line.bundleComponents) {
      ...
      for (const addOn of bundleComponent.addOns) {
        const createdAddOn =
          await billingRepository.createSaleItemBundleComponentAddOn(addOn, tx);
        if (!createdAddOn) {
...
const getCustomersInOrganization = async (
  ...
  const result = await billingRepository.getCustomersByOrganizationId(
    organizationId,
    query,
  );
  return {
...
const getSalesInStore = async (
  ...
  const [salesPage, summary] = await Promise.all([
    ...
    query.cursor
      ...
      : billingRepository.getSalesSummaryByStore(
          organizationId,
          storeId,
          normalizedQuery,
        ),
                ...
                (component) => ({
                  component: {
                    ...
                    priceAdjustmentSnapshot: Number(
                      component.priceAdjustmentSnapshot ?? 0,
                    ),
                  },
                  addOns: (component.addOns ?? []).map((addOn) => ({
                    id: addOn.id,
...
const deleteDraftSaleInStore = async (
  ...
  if (existingSale.serviceTableId) {
    return {
      status: "error",
      message:
        "Use the table's Cancel order action to discard an active table draft",
      data: null,
      code: STATUS_CODES.CONFLICT,
    };
  }
...
const commitSaleInStore = async (
  ...
  try {
    ...
    const existingSale = await buildSaleDetails(
      organizationId,
      storeId,
      saleId,
      tx,
  ...
  const sale = await billingRepository.createSale(
    {
      ...
      customerNameSnapshot: params.customer?.name ?? null,
      customerPhoneSnapshot: params.customer?.phone ?? null,
      status: "completed",
      paymentStatus:
        params.payments.length === 0
          ? "pending"
          : sumMoney(params.payments.map((payment) => payment.amount)) ===
              params.prepared.totals.grandTotal
            ? "paid"
            : "partial",
...
const replaceSaleInStore = async (
  ...
  const replacementReason =
    normalizeOptionalText(saleData.replacementReason) ?? "Bill edited";
  ...
  try {
    await pg.begin(async (tx) => {
      ...
      const updatedOriginal = await billingRepository.updateSale(
        {
          ...
          customerNameSnapshot: originalSale.customerNameSnapshot ?? null,
          customerPhoneSnapshot: originalSale.customerPhoneSnapshot ?? null,
          status: "voided",
      ...
      const originalCustomer = originalSale.customerId
        ? await getCustomerForOrganization(
            organizationId,
      ...
      const outstandingOriginalBalance = roundMoney(
        Math.max(
          moneyFrom(originalSale.grandTotal) -
            moneyFrom(originalSale.paidTotal),
          0,
        ),
      );
    ...
    if (isUniqueViolation(error)) {
      ...
      const latestOriginalSale = await buildSaleDetails(
        organizationId,
        storeId,
        originalSaleId,
      );
