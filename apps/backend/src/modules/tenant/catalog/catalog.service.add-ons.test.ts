...
describe("Add-On catalog service", () => {
  ...
  test("updates an add-on price and status", async () => {
    ...
    const response = await catalogService.updateAddOn(
      userId,
      organizationId,
      addOnId,
      {
  ...
  test("rejects deletion of an add-on with sales history", async () => {
    ...
    const response = await catalogService.deleteAddOn(
      userId,
      organizationId,
      addOnId,
    );
  ...
  test("defaults attachment selection cap to 1", async () => {
    ...
    expect(response.status).toBe("success");
    expect(
      createProductAddOnAttachmentRepo.mock.calls[0]?.[0]?.selectionCap,
    ).toBe(1);
  ...
  test("POS add-on reads return only active add-ons", async () => {
    ...
    expect(getActiveAddOnsByOrganizationId).toHaveBeenCalledWith(
      organizationId,
    );
  });
  ...
  test("POS catalog projects active Product Codes with trusted price and discount inside the device Organization", async () => {
    const codedProduct = {
      ...product,
