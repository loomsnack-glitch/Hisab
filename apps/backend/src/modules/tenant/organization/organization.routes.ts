...
router.get("/", async (c) => {
  try {
    ...
    const serviceResponse = await organizationService.getOrganizations(
      authUser.id,
    );
...
router.post(
  "/",
  validateSchema("json", CreateOrganizationSchema),
  async (c) => {
    try {
      const authUser = c.get("authUser");
      const body = c.req.valid("json");
      const serviceResponse = await organizationService.createOrganization(
        authUser.id,
        body,
      );
      return handleServiceResponse(c, serviceResponse);
...
router.patch(
  "/:organizationId",
  validateSchema("json", UpdateOrganizationSchema),
  async (c) => {
    try {
      const organizationId = c.req.param("organizationId");
      const invalidParam = validateUuidParam(
        organizationId,
      ...
      const authUser = c.get("authUser");
      const body = c.req.valid("json");
      const serviceResponse = await organizationService.updateOrganization(
        authUser.id,
        organizationId,
        body,
      );
...
router.get("/:organizationId", async (c) => {
  try {
    ...
    const authUser = c.get("authUser");
    const serviceResponse = await organizationService.getOrganizationDetails(
      authUser.id,
      organizationId,
    );
    return handleServiceResponse(c, serviceResponse);
...
router.get("/:organizationId/catalog-settings", async (c) => {
  try {
    const organizationId = c.req.param("organizationId");
    const invalidParam = validateUuidParam(
      organizationId,
    ...
    if (invalidParam) return c.json(invalidParam, invalidParam.code);

    return handleServiceResponse(
      c,
      await organizationService.getOrganizationCatalogSettings(
        c.get("authUser").id,
        organizationId,
      ),
    );
  } catch (error) {
    return handleError(FILE_NAME, "getOrganizationCatalogSettings", c, error);
  }
});

router.patch(
  "/:organizationId/catalog-settings",
  validateSchema("json", UpdateOrganizationCatalogSettingsSchema),
  async (c) => {
    try {
      const organizationId = c.req.param("organizationId");
      const invalidParam = validateUuidParam(
      ...
      return handleServiceResponse(
        c,
        await organizationService.updateOrganizationCatalogSettings(
          c.get("authUser").id,
          organizationId,
          c.req.valid("json"),
        ),
...
router.get("/:organizationId/stores", async (c) => {
  try {
    ...
    const authUser = c.get("authUser");
    const serviceResponse = await organizationService.getStores(
      authUser.id,
      organizationId,
    );
    return handleServiceResponse(c, serviceResponse);
...
router.post(
  ...
  async (c) => {
    try {
      ...
      const authUser = c.get("authUser");
      const body = c.req.valid("json");
      const serviceResponse = await organizationService.createStore(
        authUser.id,
        organizationId,
        body,
      );
...
router.get("/:organizationId/stores/:storeId/devices", async (c) => {
  try {
    ...
    const authUser = c.get("authUser");
    const serviceResponse = await organizationService.getStoreDevices(
      authUser.id,
      organizationId,
      storeId,
    );
...
router.get(
  "/:organizationId/stores/:storeId/devices/:deviceId/secret",
  async (c) => {
    try {
      const organizationId = c.req.param("organizationId");
      const storeId = c.req.param("storeId");
