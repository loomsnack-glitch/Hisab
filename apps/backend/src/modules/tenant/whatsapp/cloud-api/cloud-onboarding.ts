...
const STATE_AUDIENCE = "whatsapp-cloud-embedded-signup";
const MAX_STATE_BYTES = 4_096;
export const DEFAULT_ONBOARDING_STATE_TTL_MS = 10 * 60 * 1_000;
export const MAX_ONBOARDING_STATE_TTL_MS = 15 * 60 * 1_000;

export type CloudOnboardingStateClaims = {
...
export class CloudOnboardingStateError extends Error {
  readonly code:
  ...
  constructor(code: CloudOnboardingStateError["code"], message: string) {
    super(message);
    this.name = "CloudOnboardingStateError";
    this.code = code;
  }
}
...
type CreateCloudOnboardingStateInput = {
  organizationId: string;
  userId: string;
  secret: string;
  ttlMs?: number;
...
type VerifyCloudOnboardingStateInput = {
  token: string;
  organizationId: string;
  userId: string;
  secret: string;
...
export type CloudOnboardingReplayStore = {
...
const requireSecret = (secret: string): string => {
  ...
  if (normalized.length < 32) {
    throw new CloudOnboardingStateError(
      "invalid_configuration",
      "WhatsApp Cloud onboarding state secret is not configured",
    );
  }
  return normalized;
};
...
const requireUuid = (value: string, label: string): string => {
  ...
  if (!uuidPattern.test(normalized)) {
    throw new CloudOnboardingStateError("invalid_state", `${label} is invalid`);
  }
  return normalized;
};
...
const requireTtl = (ttlMs: number): number => {
  if (
    ...
    ttlMs < 1_000 ||
    ttlMs > MAX_ONBOARDING_STATE_TTL_MS
  ) {
    throw new CloudOnboardingStateError(
      "invalid_configuration",
      "WhatsApp Cloud onboarding state TTL is invalid",
    );
  }
  return ttlMs;
};
...
const requireNow = (now: number): number => {
  if (!Number.isSafeInteger(now) || now < 0) {
    throw new CloudOnboardingStateError(
      "invalid_state",
      "WhatsApp Cloud onboarding state clock is invalid",
    );
  }
  return now;
};
...
const base64UrlDecode = (value: string): Buffer => {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) {
    throw new CloudOnboardingStateError(
      "invalid_state",
      "WhatsApp Cloud onboarding state encoding is invalid",
    );
  }
  try {
...
const parseClaims = (encodedClaims: string): CloudOnboardingStateClaims => {
  ...
  try {
    ...
    throw new CloudOnboardingStateError(
      "invalid_state",
      "WhatsApp Cloud onboarding state is invalid",
    );
  }
...
export const createCloudOnboardingState = (
  input: CreateCloudOnboardingStateInput,
  ...
  const ttlMs = requireTtl(input.ttlMs ?? DEFAULT_ONBOARDING_STATE_TTL_MS);
  ...
  if (!noncePattern.test(nonce)) {
    throw new CloudOnboardingStateError(
      "invalid_state",
      "WhatsApp Cloud onboarding nonce is invalid",
    );
  }
...
export const verifyCloudOnboardingState = (
  input: VerifyCloudOnboardingStateInput,
): CloudOnboardingStateClaims => {
  const organizationId = requireUuid(input.organizationId, "Organization id");
  const userId = requireUuid(input.userId, "User id");
  const secret = requireSecret(input.secret);
  ...
  if (
    ...
    throw new CloudOnboardingStateError(
      "invalid_state",
      "WhatsApp Cloud onboarding state signature is invalid",
    );
  }
  ...
  if (claims.expiresAt <= now) {
    throw new CloudOnboardingStateError(
      "state_expired",
      "WhatsApp Cloud onboarding state has expired",
    );
  }
  if (claims.organizationId !== organizationId || claims.userId !== userId) {
    throw new CloudOnboardingStateError(
      "state_audience_mismatch",
      "WhatsApp Cloud onboarding state does not belong to this user",
    );
  }
  return claims;
};
...
): Promise<CloudOnboardingStateClaims> => {
  const claims = verifyCloudOnboardingState(input);
  if (!(await replayStore.consume(claims.nonce, claims.expiresAt))) {
    throw new CloudOnboardingStateError(
      "state_replayed",
      "WhatsApp Cloud onboarding state has already been used",
    );
  }
  return claims;
};
