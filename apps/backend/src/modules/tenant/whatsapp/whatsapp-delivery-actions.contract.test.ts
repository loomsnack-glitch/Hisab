...
describe("WhatsApp delivery action boundary", () => {
  test("keeps invoice status and retry on the current policy-selected account", () => {
    ...
    expect(invoiceSource).toContain("repository.getLatestInvoiceOutboxForStore");
  ...
  test("supports due status, retry, and resend for both platform and Cloud paths", () => {
    expect(repositorySource).toContain("getPlatformDueReminderOutbox");
    expect(serviceSource).toContain("getLatestDueReminderOutboxForStore");
