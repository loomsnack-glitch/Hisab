...
import type { CreatePurchaseJSON, DeviceSessionDTO, PurchaseDetailDTO, PurchaseSummaryDTO } from "@repo/types";
...
const PosPurchasesPanel = ({ session, search }: PosPurchasesPanelProps) => {
    ...
    const purchases: PurchaseSummaryDTO[] = purchasesQuery.data?.status === "success" ? purchasesQuery.data.data?.purchases ?? [] : [];
