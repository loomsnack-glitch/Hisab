...
import { Armchair, Check, CircleOff, LayoutGrid, Users } from "lucide-react";
...
const stateClassName: Record<ServiceTableDTO["state"], string> = {
  ...
  allocated: "border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300",
  engaged: "border-sky-500/40 bg-sky-500/10 text-sky-700 dark:text-sky-300",
  ready_to_bill: "border-violet-500/40 bg-violet-500/10 text-violet-700 dark:text-violet-300",
  payment_due: "border-orange-500/40 bg-orange-500/10 text-orange-700 dark:text-orange-300",
...
const PosTablesPage = () => {
  ...
  const operationMutation = useMutation({
    mutationFn: ({ tableId, action }: TableOperation) =>
      action === "allocate" ? allocatePosServiceTable(tableId) : freePosServiceTable(tableId),
    onSuccess: async (response, variables) => {
      if (response.status !== "success") {
        toast.error(response.message);
        return;
      }
      await queryClient.invalidateQueries({ queryKey: serviceTableKeys.pos(session.organization.id, session.store.id) });
      toast.success(variables.action === "allocate" ? "Table allocated" : "Table freed");
    },
    onError: (error: { message?: string }) => toast.error(error.message ?? "Table operation failed"),
  });

  const handleTableOperation = (tableId: string, action: PosServiceTableAction) => {
      ...
      <section className="min-h-0 min-w-0 flex-1 overflow-y-auto p-4 pb-8 lg:p-6">
        <div className="mx-auto max-w-7xl space-y-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-medium text-primary">{tablesQuery.data?.status === "success" ? "Live service area" : "Service area"}</p>
              <h1 className="font-display text-3xl font-semibold tracking-tight">Tables</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {tablesQuery.data?.status === "success" ? `Manage the current floor for ${session.store.name}.` : "Loading your Store floor..."}
              </p>
            </div>
          ...
          <Card className="overflow-hidden border-border/60 bg-card/80 shadow-sm">
            <CardHeader className="border-b border-border/50 bg-muted/10">
              <div className="flex items-center gap-3">
                <div className="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Armchair className="size-5" /></div>
                <div>
                  <CardTitle className="text-lg">{session.store.name} floor</CardTitle>
                  <CardDescription>Free and Allocated tables can be changed before an order exists.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-3 sm:p-5">
              {tablesQuery.isPending ? (
                ...
                <Empty className="min-h-80 rounded-2xl border border-dashed">
                  <EmptyHeader><EmptyMedia variant="icon"><Armchair /></EmptyMedia><EmptyTitle>No tables configured</EmptyTitle><EmptyDescription>Ask an administrator to configure Service Tables for this Store.</EmptyDescription></EmptyHeader>
                </Empty>
              ) : (
                <div className="relative min-h-[32rem] overflow-hidden rounded-2xl border border-dashed border-border/70 bg-[radial-gradient(circle,_var(--color-border)_1px,_transparent_1px)] [background-size:24px_24px]">
                  {tables.map((table) => {
                    ...
                    const isOperating = operationMutation.isPending && operationMutation.variables?.tableId === table.id;
                    return (
                      <div
                        key={table.id}
                        data-testid={`pos-table-${table.id}`}
                        className="absolute flex flex-col items-center"
                        style={{ ...tablePositionStyle(table.position), width: TABLE_BOX_SIZE.width + 24 }}
                      >
                        <div className={`flex h-16 w-32 flex-col items-center justify-center rounded-2xl border-2 bg-card shadow-md ${stateClassName[table.state]}`}>
                          <span className="font-display text-lg font-semibold text-foreground">{table.tableLabel}</span>
                          <Badge variant="outline" className={`mt-1 ${stateClassName[table.state]}`}>{getPosServiceTableStateLabel(table.state)}</Badge>
                        </div>
                        ...
                        {action ? (
                          <Button
                            ...
                            onClick={() => handleTableOperation(table.id, action)}
